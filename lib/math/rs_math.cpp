...
static string fToString(double f, int precision = 3)
{
  char buf[32] = { 0 };
  snprintf(buf, 31, "%.*f", precision, f);
  return string(buf);
}

string RS_Math::doubleToString(double value, double prec) {
  if (prec < RS_TOLERANCE) {
    //RS_DEBUG->print(RS_Debug::D_ERROR,
    //  "RS_Math::doubleToString: invalid precision");
    //return string().setNum(value, prec);
    return fToString(value);
  }

  double const num = RS_Math::round(value / prec) * prec;

  string exaStr = RS_Math::doubleToString(1. / prec, 10);
  int const dotPos = exaStr.find('.');

  if (dotPos == -1) {
    //big numbers for the precision
    //return QString().setNum(RS_Math::round(num));
    return fToString(RS_Math::round(num));
  }
  else {
    //number of digits after the point
    int digits = dotPos - 1;
 ...
 */
string RS_Math::doubleToString(double value, int prec) {
    string valStr;
    ...
    valStr = fToString(value, prec);
    size_t t = valStr.rfind('.');
    if (t > 0) {
          while (valStr.length() - t > prec)
            valStr.at(valStr.length() - t - 1) = 0;

          if(valStr.at(valStr.length()-1)=='.') {
              valStr.at(valStr.length()-1) = 0;
          }
    }
...
bool RS_Math::simultaneousQuadraticVerify(const std::vector<std::vector<double> >& m, RS_Vector& v)
{
    ...
    for(size_t i0=0; i0<20; ++i0){
        ...
        if(!ret) break;
        v -= RS_Vector(dn[0], dn[1]).asVector();
    }
