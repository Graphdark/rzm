...
getSysVar(LPCTSTR varName, int& val)
{
  ...
  if (acedGetVar(varName, &rb)== RTNORM) {
    ...
    case RTLONG:
      ...
      return(Acad::eOk);
    case RTT:
      val = rb.resval.rint == 0 ? 0 : 1;
      return(Acad::eOk);
    default:
      ASSERT(rb.restype == RTSHORT || rb.restype == RTLONG || rb.restype == RTT);
    }
    return(Acad::eInvalidInput);
  }
  else
...
{
    resbuf rb;
    // !!! Почему-то вернула RT3DPOINT! !!!
    if (acedGetVar(varName, &rb) == RTNORM) {
        ASSERT(rb.restype == RTPOINT || rb.restype == RT3DPOINT);
